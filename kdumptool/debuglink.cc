...
gnu_debuglink_crc32 (uint32_t crc, unsigned char *buf, size_t len)
{
    ...
    : m_rootdir(rootdir), m_filename(file)
{
    ...
    FilePath rootkernel = m_rootdir;
    rootkernel.appendPath(file);
    if (!rootkernel.exists())
        throw KError("File " + rootkernel + " does not exist.");
}

// -----------------------------------------------------------------------------
void Debuglink::readDebuglink()
    throw (KError)
{
    Debug::debug()->trace("Debuglink::readDebuglink()");
    ...
    try {
        ...
        fp.appendPath(m_filename);
        if (Util::isGzipFile(fp.c_str()))
            fd = openCompressed();
        else
            fd = openUncompressed();

        elf = openElf(fd);
        readDebuginfoLink(elf);
    ...
    throw (KError)
{
    ...
    (path = m_filename.dirName()).appendPath(m_debuglink);
    (rootpath = m_rootdir).appendPath(path);
    try {
    ...
    (path = m_filename.dirName()).appendPath(".debug").appendPath(m_debuglink);
    (rootpath = m_rootdir).appendPath(path);
    try {
        uint32_t crc = calcCrc(rootpath);
        if (crc != m_crc)
    ...
    (path = GLOBALDEBUGDIR).appendPath(m_filename.dirName()).appendPath(m_debuglink);
    (rootpath = m_rootdir).appendPath(path);
    try {
        uint32_t crc = calcCrc(rootpath);
        if (crc != m_crc)
    ...
    throw (KError)
{
    ...
    FilePath templ = Util::getenv("TMPDIR", "/tmp");
    templ.appendPath("read-debuglink-XXXXXX");
    templbuffer = strdupa(templ.c_str());

    Debug::debug()->trace("Debuglink::openCompressed()");

    FilePath rootfile;
    (rootfile = m_rootdir).appendPath(m_filename);
    ...
    while ((read_chars = gzread(source, buffer, BUFSIZ)) > 0) {
        if (write(dest, buffer, read_chars) != read_chars)
            throw KSystemError("Write to " + string(templ) + " failed.", errno);
    }

    gzclose(source);

    // don't close dest here!
    off_t ret = lseek(dest, SEEK_SET, 0);
    if (ret == (off_t)-1)
        throw KSystemError("lseek() to " + string(templ) + " failed.", errno);
...
// -----------------------------------------------------------------------------
int Debuglink::openUncompressed()
    throw (KError)
{
    int fd;
...
// -----------------------------------------------------------------------------
Elf *Debuglink::openElf(int fd)
    throw (KError)
{
    GElf_Ehdr elfhdr;
    Elf *elf;

    Debug::debug()->trace("Debuglink::openElf(%d)", fd);

    elf = elf_begin(fd, ELF_C_READ, NULL);
    if (!elf)
        throw KELFError("elf_begin() failed.", elf_errno());

    if (gelf_getehdr(elf, &elfhdr) == 0)
        throw KELFError("gelf_getehdr failed", elf_errno());

    if (elf_kind(elf) != ELF_K_ELF)
        throw KELFError("elf_kind failed.", elf_errno());

    if (elfhdr.e_type != ET_EXEC && elfhdr.e_type != ET_DYN)
        throw KELFError("Wrong ELF type", elf_errno());

    return elf;
}

// -----------------------------------------------------------------------------
void Debuglink::readDebuginfoLink(Elf *elf)
    throw (KError)
{
    ...
    while ((scn = elf_nextscn(elf, scn)) != NULL) {
        ...
        if (gelf_getshdr(scn, &shdr) == NULL)
            KELFError("Can't get section header.", elf_errno());
    ...
    if (!found)
        throw KError(".gnu_debuglink was not found.");
    ...
    try {
        while (n < (int)shdr.sh_size && (data = elf_getdata(scn, data)) != NULL) {

            /* merge */
            debuglink_data = (char *)realloc(debuglink_data,
                    debuglink_data_size + data->d_size);
            if (!debuglink_data)
                throw KError("Unable to (re-)allocate memory.");
        ...
        ByteVector bv;
        bv.insert(bv.end(), debuglink_data,
                            debuglink_data + strlen(debuglink_data));
        m_debuglink = Stringutil::bytes2str(bv);
    } catch (...) {
        free(debuglink_data);
        throw;
    }
    ...
    int offset = strlen(debuglink_data) + 1;
    offset += 3;
    offset &= ~3;

    memcpy(&m_crc, debuglink_data + offset, 4);
}

// -----------------------------------------------------------------------------
uint32_t Debuglink::calcCrc(const string &file)
    throw (KError)
{
    unsigned char buffer[BUFSIZ];
    int ret;

    Debug::debug()->trace("Debuglink::calcCrc(%s)", file.c_str());
...
// -----------------------------------------------------------------------------
string Debuglink::getDebuglink() const
    throw (KError)
{
    if (m_debuglink.size() == 0)
        throw KError("Debuglink::readDebuglink() not called.");
...
// -----------------------------------------------------------------------------
uint32_t Debuglink::getCrc() const
    throw (KError)
{
    if (m_debuglink.size() == 0)
        throw KError("Debuglink::readDebuglink() not called.");
